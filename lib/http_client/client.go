...
import (
	...
	"github.com/lristar/go-boot/lib/stringx"
...
func NewRequest() *MyRequest {
	r := &MyRequest{Timeout: 120, Alert: true, Tr: new(http.Transport)}
	return r
}
...
// HTTPDoWeb 发起WEB请求
func (r *MyRequest) HTTPDoWeb(data string) ([]byte, error) {
	client := &http.Client{Timeout: r.Timeout * time.Second, Transport: r.Tr}
	if r.Method == "" {
		r.Method = "GET"
	}

	req, err := http.NewRequest(r.Method, r.URL, strings.NewReader(data))
	if err != nil {
		return nil, err
	}
	for k, v := range r.Header {
	...
	if r.Ctx != nil {
		...
		if ok {
			...
			span.LogFields(tracerLog.String("http.req.body", data))
			span.SetTag("http.req.url", r.URL)
		}
	}
	resp, err := client.Do(req)
